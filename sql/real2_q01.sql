...
--    เก็บกวาดด้วยมือ:  DROP INDEX IF EXISTS qf_real2_hnsw;
...
DROP TABLE IF EXISTS qf_real2_q01;
CREATE TABLE qf_real2_q01 (
    label        text,
    build_no     int,
    build_ms     numeric,
    index_size   text,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_real2_measure(
...
BEGIN
    ...
    SELECT * INTO r10  FROM qf_real2_recall_at(10);
    SELECT * INTO r100 FROM qf_real2_recall_at(100);

    INSERT INTO qf_real2_q01 VALUES (
...
BEGIN
    ...
    PERFORM qf_real2_measure('exact', 0, 0, '-');
END $$;
...
BEGIN
    ...
    FOR i IN 1..n_builds LOOP
        ...
        ms := extract(epoch FROM clock_timestamp() - t0) * 1000;
        sz := pg_size_pretty(pg_relation_size('qf_real2_hnsw'));
        RAISE NOTICE '--- HNSW build %/% (% วิ) ---', i, n_builds, round(ms/1000);
        PERFORM qf_real2_measure('HNSW ef_search=40', i, ms, sz);
    END LOOP;
    EXECUTE 'DROP INDEX IF EXISTS qf_real2_hnsw';
END $$;
...
BEGIN
    ...
    FOR i IN 1..n_builds LOOP
        ...
        ms := extract(epoch FROM clock_timestamp() - t0) * 1000;
        sz := pg_size_pretty(pg_relation_size('qf_real2_ivf'));
        RAISE NOTICE '--- IVFFlat build %/% (% วิ) ---', i, n_builds, round(ms/1000);
        PERFORM qf_real2_measure('IVFFlat probes=1', i, ms, sz);
    END LOOP;
    EXECUTE 'DROP INDEX IF EXISTS qf_real2_ivf';
END $$;
...
BEGIN
    SELECT recall_10 INTO e FROM qf_real2_q01 WHERE label = 'exact';
    ...
    SELECT avg(recall_10) INTO h FROM qf_real2_q01 WHERE label LIKE 'HNSW%';
    SELECT avg(recall_10) INTO v FROM qf_real2_q01 WHERE label LIKE 'IVFFlat%';
