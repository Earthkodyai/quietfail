...
DROP INDEX IF EXISTS qf_q03r2_idx;
DROP TABLE IF EXISTS qf_q03r2_fix;
DROP TABLE IF EXISTS qf_q03r2_sweep;
DROP TABLE IF EXISTS qf_q03r2;

CREATE TABLE qf_q03r2 (id bigint PRIMARY KEY, grp int, embedding vector(768));
INSERT INTO qf_q03r2 SELECT id, id % 1000, embedding FROM qf_real2;

CREATE TABLE qf_q03r2_sweep (
    sel_pct numeric, rows_matching bigint, asked int,
    got_index int, got_exact int, uses_index bool, buffers bigint
);
CREATE TABLE qf_q03r2_fix (
...
CREATE INDEX qf_q03r2_idx ON qf_q03r2 USING hnsw (embedding vector_cosine_ops);
ANALYZE qf_q03r2;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_q03r2_probe(p_lt int, p_limit int) RETURNS void
LANGUAGE plpgsql AS $$
DECLARE
...
BEGIN
    ...
    PERFORM (SELECT count(*) FROM (
        SELECT id FROM qf_q03r2
        ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT 10) w);

    SELECT count(*) INTO n_match FROM qf_q03r2 WHERE grp < p_lt;

    EXECUTE format(
        'SELECT count(*) FROM (SELECT id FROM qf_q03r2 WHERE grp < %s '
        'ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT %s) s',
    ...
    EXECUTE format(
        'EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT id FROM qf_q03r2 WHERE grp < %s '
        'ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT %s',
        p_lt, p_limit) INTO j;

    -- กฎเหล็กข้อ 6: อ่านโครงสร้าง ต้องระบุชื่อ index ไม่ใช่ค้นคำว่า "Index Scan"
    uses := j::text LIKE '%qf_q03r2_idx%';
...
SELECT qf_q03r2_probe(1000, 10);
SELECT qf_q03r2_probe(500, 10);
SELECT qf_q03r2_probe(200, 10);
SELECT qf_q03r2_probe(100, 10);
SELECT qf_q03r2_probe(50, 10);
SELECT qf_q03r2_probe(20, 10);
SELECT qf_q03r2_probe(10, 10);
SELECT qf_q03r2_probe(5, 10);
SELECT qf_q03r2_probe(2, 10);
SELECT qf_q03r2_probe(1, 10);
...
FROM qf_q03r2_sweep ORDER BY sel_pct DESC;
...
CREATE OR REPLACE FUNCTION qf_q03r2_fixstep(p_step text, p_desc text) RETURNS void
LANGUAGE plpgsql AS $$
DECLARE n bigint; j json; buf bigint;
BEGIN
    SELECT count(*) INTO n FROM (
        SELECT id FROM qf_q03r2 WHERE grp < 10
        ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT 40) s;
    EXECUTE 'EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT id FROM qf_q03r2 WHERE grp < 10 '
            'ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT 40'
        INTO j;
    buf := coalesce((j -> 0 -> 'Plan' ->> 'Shared Hit Blocks')::bigint, 0)
         + coalesce((j -> 0 -> 'Plan' ->> 'Shared Read Blocks')::bigint, 0);
    INSERT INTO qf_q03r2_fix VALUES (p_step, p_desc, 40, n, buf);
END $$;

SELECT qf_q03r2_fixstep('1', 'ค่าเริ่มต้นทั้งหมด (iterative_scan = off)');

SET hnsw.iterative_scan = relaxed_order;
SELECT qf_q03r2_fixstep('2', 'เปิด iterative_scan = relaxed_order');

SET hnsw.max_scan_tuples = 200000;
SELECT qf_q03r2_fixstep('3', '+ max_scan_tuples 20000 -> 200000');

RESET hnsw.max_scan_tuples;
SET hnsw.scan_mem_multiplier = 8;
SELECT qf_q03r2_fixstep('4', '+ scan_mem_multiplier 1 -> 8');
...
FROM qf_q03r2_fix ORDER BY step;
...
BEGIN
    ...
    SELECT count(*) INTO n_exact_bad FROM qf_q03r2_sweep WHERE got_exact <> asked;
    ...
    SELECT count(*) INTO n_bad FROM qf_q03r2_sweep WHERE got_index < asked;
    RAISE NOTICE '[2/2] ระดับที่ index ให้ไม่ครบ: % จาก % ระดับ',
        n_bad, (SELECT count(*) FROM qf_q03r2_sweep);
END $$;
...
--    ตารางเหล่านี้ใหญ่มาก (qf_q03r2 ราว 400 MB บนชุด 768 มิติ) และค้างสะสม
