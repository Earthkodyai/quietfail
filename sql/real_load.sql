-- ============================================================
-- โหลด corpus จาก embedding จริง เข้าฐานข้อมูล + สร้างเฉลย
--
...
--   sql/_real_corpus.tsv · sql/_real_queries.tsv
--
...
-- ------------------------------------------------------------
DROP TABLE IF EXISTS qf_real_guard;
...
CREATE TEMP TABLE qf_real_guard AS
...
DECLARE g record;
BEGIN
    SELECT * INTO g FROM qf_real_guard;
    IF g.rows <> 5000 THEN
...
-- ------------------------------------------------------------
DROP TABLE IF EXISTS qf_real_truth;
DROP TABLE IF EXISTS qf_real_q;
DROP TABLE IF EXISTS qf_real;

CREATE TABLE qf_real (
    id        bigint PRIMARY KEY,
    body      text   NOT NULL,
    embedding vector(384)
);

CREATE TABLE qf_real_q (
    id        int PRIMARY KEY,
    body      text NOT NULL,
    embedding vector(384)
);

CREATE TABLE qf_real_truth (
    query_id int,
    k        int,
    ids      bigint[],
...
\copy qf_real   (id, body, embedding) FROM '/sql/_real_corpus.tsv'  WITH (FORMAT csv, DELIMITER E'\t', QUOTE E'\x01')
\copy qf_real_q (id, body, embedding) FROM '/sql/_real_queries.tsv' WITH (FORMAT csv, DELIMITER E'\t', QUOTE E'\x01')
...
BEGIN
    SELECT count(*) INTO n_c FROM qf_real;
    SELECT count(*) INTO n_q FROM qf_real_q;
    IF n_c <> 100000 THEN
    ...
    SELECT vector_dims(embedding) INTO d_c FROM qf_real LIMIT 1;
    SELECT vector_dims(embedding) INTO d_q FROM qf_real_q LIMIT 1;
    IF d_c <> 384 OR d_q <> 384 THEN
        RAISE EXCEPTION 'มิติไม่ตรง: corpus % · query % (ต้อง 384)', d_c, d_q;
    END IF;

    -- V07 สอนไว้แล้วว่า NULL/zero vector ทำให้แถวหายเงียบๆ ต้องกันตั้งแต่ต้น
    SELECT count(*) INTO n_null FROM qf_real
    ...
    RAISE NOTICE 'โหลดแล้ว corpus % แถว · query % แถว · 384 มิติ · ไม่มี NULL/zero',
        n_c, n_q;
END $$;
...
-- ------------------------------------------------------------
DROP TABLE IF EXISTS qf_real_sanity;
CREATE TABLE qf_real_sanity AS
WITH q AS (SELECT id, embedding FROM qf_real_q ORDER BY id LIMIT 20),
nn AS (
    ...
    LATERAL (
        ...
        FROM (SELECT c.embedding <=> q.embedding AS dist
              FROM qf_real c ORDER BY c.embedding <=> q.embedding LIMIT 100) d
    ) t
)
SELECT
...
DECLARE s record;
BEGIN
    SELECT * INTO s FROM qf_real_sanity;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_real_build_truth(p_k int)
RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE
    n_done bigint := 0;
...
BEGIN
    ...
    EXECUTE format(
        'EXPLAIN (FORMAT JSON) SELECT id FROM qf_real '
    ...
    DELETE FROM qf_real_truth WHERE k = p_k;

    FOR q IN SELECT id, embedding FROM qf_real_q ORDER BY id LOOP
        INSERT INTO qf_real_truth (query_id, k, ids)
...
CREATE OR REPLACE FUNCTION qf_real_recall_at(p_k int)
RETURNS TABLE (
    k int, n_queries int, mean_recall numeric, min_recall numeric, n_perfect int
)
LANGUAGE plpgsql AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM qf_real_truth WHERE qf_real_truth.k = p_k) THEN
    ...
    WITH per_query AS (
        SELECT t.query_id,
            ...
            AS recall
        FROM qf_real_truth t
        JOIN qf_real_q q ON q.id = t.query_id
        WHERE t.k = p_k
    )
    SELECT p_k,
           count(*)::int,
...
SELECT qf_real_build_truth(10)  AS "สร้างเฉลย k=10";
SELECT qf_real_build_truth(100) AS "สร้างเฉลย k=100";
...
SELECT count(*) AS "corpus จริง", pg_size_pretty(pg_total_relation_size('qf_real')) AS "ขนาด"
FROM qf_real;
