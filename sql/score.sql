...
BEGIN
    ...
    LOOP
        ...
        --
        -- 📌 ตั้งใจคงไว้แบบนี้ แม้จะมีสูตรจากซอร์สแล้ว (2026-08-01)
        --    สูตร: ความจุ = (งบ) / (674 + MAXALIGN(8 + 4*dim))  ดู results/i05_capacity_model.txt
        --
        --    ไฟล์นี้เป็นตัวนับคะแนนของ **งานวิจัย** วิ่งบน qf_corpus ซึ่งล็อกไว้ที่
        --    384 มิติ + mwm 64MB = จุดที่ 443 tuples/MB calibrate ไว้พอดี
        --    การจงใจใช้ค่าต่ำเพื่อ "เตือนเร็วกว่าจริงเล็กน้อย" จึงเหมาะกับบทบาทนี้
        --
        --    🔴 แต่ **ของส่งมอบใช้ค่านี้ไม่ได้** — scripts/quietfail_check.py เคยใช้
        --    ค่าเดียวกันแล้วเกิดผลบวกลวงที่ mwm อื่น (128MB ประเมินต่ำไป 3.3%)
        --    เพราะงบจริงคือ mwm - 4MiB ซึ่งหักคงที่ ค่าต่อ MB จึงไม่คงที่
        --    ตัวนั้นเปลี่ยนไปใช้สูตรแล้ว · **เจตนาต่างกัน: ที่นี่เตือนเร็วดีกว่า
        --    ที่โน่นเตือนผิดไม่ได้**
        ELSIF fid = 'I05' THEN
            rel        := exp ->> 'target_table';
