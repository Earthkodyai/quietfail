-- ============================================================
-- Q06 บน embedding จริง 768 มิติ — วัดตำแหน่งเพดาน
--
-- 🔴 ผลที่ได้: เพดาน **41 เท่ากันทั้ง 384 และ 768 มิติ** (ขอ 50 · 100 · 1000 ได้ 41)
--    -> เป็นค่าเชิงโครงสร้าง ไม่ขึ้นกับทั้งข้อมูลและมิติ
--    และ E44 พบภายหลังว่า hnsw.iterative_scan ยกเพดานได้หมด แต่ recall เหลือ 0.7572
--    (เติม 2026-08-02 ให้ตรงกับฝาแฝด real_q06.sql — กับดักข้อ 14บ)
--
...
--    ถ้าถูกตัดกลางคันจะทิ้ง index ค้างบนตารางที่ล็อกไว้ (กับดักข้อ 4 · 14ธ)
--    เก็บกวาดด้วยมือ:  DROP INDEX IF EXISTS qf_q06r2_idx;
--    แล้วยืนยันด้วย    python scripts/audit.py
-- ============================================================
\timing on
\set ON_ERROR_STOP on

DROP INDEX IF EXISTS qf_q06r2_idx;
DROP TABLE IF EXISTS qf_q06r2_obs;

CREATE TABLE qf_q06r2_obs (ef int, asked int, got bigint, got_exact bigint);
...
CREATE INDEX qf_q06r2_idx ON qf_real2 USING hnsw (embedding vector_cosine_ops);
ANALYZE qf_real2;

CREATE OR REPLACE FUNCTION qf_q06r2_probe(p_ef int, p_limit int) RETURNS void
LANGUAGE plpgsql AS $$
...
BEGIN
    ...
    EXECUTE format('SELECT count(*) FROM (SELECT id FROM qf_real2 '
    ...
    INSERT INTO qf_q06r2_obs VALUES (p_ef, p_limit, n, n_exact);
END $$;

\qecho ''
\qecho '=== ef_search = 40 (ค่าเริ่มต้น) · ไล่ค่า LIMIT รอบหน้าผา ==='
SELECT qf_q06r2_probe(40, 10);
SELECT qf_q06r2_probe(40, 39);
SELECT qf_q06r2_probe(40, 40);
SELECT qf_q06r2_probe(40, 41);
SELECT qf_q06r2_probe(40, 50);
SELECT qf_q06r2_probe(40, 100);
SELECT qf_q06r2_probe(40, 1000);

\qecho ''
\qecho '=== ขอ 100 แถว · ไล่ค่า ef_search ==='
SELECT qf_q06r2_probe(100, 100);
SELECT qf_q06r2_probe(200, 100);

SELECT ef AS "ef_search", asked AS "ขอ", got AS "ได้", got_exact AS "เฉลย",
       CASE WHEN got < asked THEN 'ขาด ' || (asked - got) ELSE 'ครบ' END AS "ผล"
FROM qf_q06r2_obs ORDER BY ctid;
...
BEGIN
    SELECT got INTO g39   FROM qf_q06r2_obs WHERE ef=40 AND asked=39;
    SELECT got INTO g40   FROM qf_q06r2_obs WHERE ef=40 AND asked=40;
    SELECT got INTO g41   FROM qf_q06r2_obs WHERE ef=40 AND asked=41;
    SELECT got INTO g100  FROM qf_q06r2_obs WHERE ef=40 AND asked=100;
    SELECT got INTO g1000 FROM qf_q06r2_obs WHERE ef=40 AND asked=1000;
...
-- ============================================================
-- เก็บกวาด + guard ปิดท้าย (เพิ่ม 2026-08-02 · กับดักข้อ 14ฐ)
-- ============================================================
DO $$
...
DROP TABLE IF EXISTS qf_q06r2_obs CASCADE;
