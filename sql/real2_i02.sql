...
--
-- ⚠️ ไม่แตะ qf_corpus และ qf_real2 — ทำงานบนสำเนา qf_i02r2 เท่านั้น
-- ============================================================
\timing on
...
-- ⭐ guard ตารางต้นทาง — ไฟล์นี้อ่าน qf_real2 อย่างเดียว ห้ามเขียน (เพิ่ม 2026-08-02)
DROP TABLE IF EXISTS qf_i02r2_guard;
CREATE TABLE qf_i02r2_guard AS SELECT count(*) AS n FROM qf_real2;

DROP TABLE IF EXISTS qf_i02r2_obs;
DROP TABLE IF EXISTS qf_i02r2_meta;
DROP TABLE IF EXISTS qf_i02r2;

CREATE TABLE qf_i02r2 (id bigint PRIMARY KEY, embedding vector(768));

CREATE TABLE qf_i02r2_meta (
    round        int,
    cond         text,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_i02r2_measure(
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_i02r2_run(p_round int, p_cond text, p_rows bigint)
RETURNS void
LANGUAGE plpgsql AS $$
DECLARE t0 timestamptz; ms numeric;
BEGIN
    EXECUTE 'DROP INDEX IF EXISTS qf_i02r2_idx';
    TRUNCATE qf_i02r2;

    INSERT INTO qf_i02r2 SELECT id, embedding FROM qf_real2 ORDER BY id LIMIT p_rows;

    t0 := clock_timestamp();
    EXECUTE 'CREATE INDEX qf_i02r2_idx ON qf_i02r2 '
            'USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)';
    ms := extract(epoch FROM clock_timestamp() - t0) * 1000;

    -- เติมให้ครบ 100,000 — index ถูกสร้างไปแล้วจากตัวอย่างชุดแรกเท่านั้น
    INSERT INTO qf_i02r2
    SELECT c.id, c.embedding FROM qf_real2 c
    WHERE NOT EXISTS (SELECT 1 FROM qf_i02r2 x WHERE x.id = c.id);

    PERFORM qf_i02r2_measure(p_round, p_cond, p_rows, ms);
    EXECUTE 'DROP INDEX IF EXISTS qf_i02r2_idx';
END $$;
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_i02r2_run_narrow(p_round int, p_rows bigint)
RETURNS void
LANGUAGE plpgsql AS $$
DECLARE t0 timestamptz; ms numeric; anchor vector(768);
BEGIN
    EXECUTE 'DROP INDEX IF EXISTS qf_i02r2_idx';
    TRUNCATE qf_i02r2;

    SELECT embedding INTO anchor FROM qf_real2 ORDER BY id LIMIT 1;

    INSERT INTO qf_i02r2
    SELECT id, embedding FROM qf_real2 ORDER BY embedding <=> anchor LIMIT p_rows;
    ...
    PERFORM qf_i02r2_measure(p_round, 'D แคบ', p_rows, ms);
    EXECUTE 'DROP INDEX IF EXISTS qf_i02r2_idx';
END $$;
...
BEGIN
    ...
    FOR r IN 1..n LOOP
        ...
        PERFORM qf_i02r2_run(r, 'A 50',      50);
        PERFORM qf_i02r2_run(r, 'B 1000',    1000);
        PERFORM qf_i02r2_run(r, 'C 100000',  100000);
        PERFORM qf_i02r2_run_narrow(r,       1000);
    END LOOP;
END $$;
...
BEGIN
    ...
    SELECT count(DISTINCT final_fp), count(DISTINCT final_rows)
      INTO n_fp, n_rows FROM qf_i02r2_meta;
...
FROM qf_i02r2_meta m
JOIN qf_i02r2_obs o ON o.round = m.round AND o.cond = m.cond
JOIN LATERAL (SELECT avg(o2.recall) AS r FROM qf_i02r2_obs o2
...
--    ตาราง qf_i02r2 เป็นสำเนา embedding ขนาดราว 400 MB จึงค้างสะสมทุกครั้งที่รัน
...
BEGIN
    SELECT n INTO n_before FROM qf_i02r2_guard;
...
DROP TABLE IF EXISTS qf_i02r2_guard;
DROP FUNCTION IF EXISTS qf_i02r2_measure(int, text, bigint, numeric);
DROP TABLE IF EXISTS qf_i02r2_obs  CASCADE;
DROP TABLE IF EXISTS qf_i02r2_meta CASCADE;
