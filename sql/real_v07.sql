...
-- ⚠️ แถวปกติมาจาก qf_real (embedding จริง) ไม่ใช่เวกเตอร์ที่ปั้นเอง
-- ============================================================
\timing on
\set ON_ERROR_STOP on

DROP INDEX IF EXISTS qf_v07r_idx;
DROP TABLE IF EXISTS qf_v07r;

CREATE TABLE qf_v07r (id bigint PRIMARY KEY, kind text, embedding vector(384));

-- 500 แถวปกติจาก embedding จริง
INSERT INTO qf_v07r SELECT id, 'ปกติ', embedding FROM qf_real ORDER BY id LIMIT 500;

-- 5 NULL
INSERT INTO qf_v07r
SELECT 1000000 + g, 'NULL', NULL FROM generate_series(1, 5) g;

-- 5 zero vector — เวกเตอร์ศูนย์ 384 มิติ
INSERT INTO qf_v07r
SELECT 2000000 + g, 'zero',
       (SELECT ('[' || string_agg('0', ',') || ']')::vector FROM generate_series(1, 384))
FROM generate_series(1, 5) g;

DO $$
DECLARE n bigint;
BEGIN
    SELECT count(*) INTO n FROM qf_v07r;
    IF n <> 510 THEN
...
SELECT ('[' || string_agg('0', ',') || ']')::vector <=> (SELECT embedding FROM qf_real_q WHERE id=0)
       AS "zero <=> query"
FROM generate_series(1, 384);
SELECT NULL::vector(384) <=> (SELECT embedding FROM qf_real_q WHERE id=0) AS "NULL <=> query";

-- ------------------------------------------------------------
-- ตัววัด 3 เส้นทาง
-- ------------------------------------------------------------
DROP TABLE IF EXISTS qf_v07r_obs;
CREATE TABLE qf_v07r_obs (
...
DROP FUNCTION IF EXISTS qf_v07r_probe(text, boolean);
CREATE FUNCTION qf_v07r_probe(p_path text, p_use_index bool) RETURNS void
LANGUAGE plpgsql AS $$
DECLARE qvec vector; j json; used bool;
BEGIN
    SELECT embedding INTO qvec FROM qf_real_q WHERE id = 0;
    ...
    EXECUTE 'EXPLAIN (FORMAT JSON) SELECT kind FROM qf_v07r '
            'ORDER BY embedding <=> $1 LIMIT 510'
      INTO j USING qvec;
    used := j::text LIKE '%qf_v07r_idx%';

    INSERT INTO qf_v07r_obs
    SELECT p_path, 510, count(*),
           count(*) FILTER (WHERE kind = 'ปกติ'),
           count(*) FILTER (WHERE kind = 'NULL'),
           count(*) FILTER (WHERE kind = 'zero'),
           used
    FROM (SELECT kind FROM qf_v07r ORDER BY embedding <=> qvec LIMIT 510) s;
...
SELECT qf_v07r_probe('1. ไม่มี index (exact)', false);

-- สร้าง index แล้ววัดอีกสองเส้นทาง
SET hnsw.ef_search = 1000;
CREATE INDEX qf_v07r_idx ON qf_v07r USING hnsw (embedding vector_cosine_ops);
ANALYZE qf_v07r;

-- เส้นทางที่ 2 — **กลุ่มควบคุม** index มีอยู่ แต่บังคับไม่ให้ใช้
SELECT qf_v07r_probe('2. มี index แต่ปิด index scan', false);

-- เส้นทางที่ 3 — ใช้ index จริง
SELECT qf_v07r_probe('3. ใช้ index', true);
...
FROM qf_v07r_obs ORDER BY path;
...
BEGIN
    SELECT got INTO g1 FROM qf_v07r_obs WHERE path LIKE '1.%';
    SELECT got INTO g2 FROM qf_v07r_obs WHERE path LIKE '2.%';
    SELECT got INTO g3 FROM qf_v07r_obs WHERE path LIKE '3.%';
    ...
    IF NOT (SELECT used_vector_index FROM qf_v07r_obs WHERE path LIKE '3.%') THEN
        RAISE EXCEPTION
