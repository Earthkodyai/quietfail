...
--         psql -U lab -d faultlab -v ON_ERROR_STOP=1 -v builds=3 -f //sql/real2_q04.sql
--
...
--
-- 🔴 ผลที่ได้: **ข้ออ้างข้างบนถูกถอน** — บนข้อมูลจริง 384 มิติ probes=5 ได้ 0.9068
--    และ probes=10 ตามสูตรเอกสารได้ 0.9527 ซึ่งดีกว่าจริง -> เอกสารแนะนำเหมาะสม
--    ⚠️ และ E45 พบว่าแถว probes สูง planner เลิกใช้ index เอง ตามที่เอกสารเตือนไว้
--       ("the planner won't use the index") · บน 768 มิติ planner ทิ้ง index
--       ตั้งแต่ probes = 50 ซึ่งเร็วกว่า 384 มิติ (ทิ้งที่ 100)
--       -> เทียบเวลาข้ามแถวเหล่านั้นไม่ได้
--
-- 🔴 กล่องนี้เพิ่ม 2026-08-02 — `real_q04.sql` ได้กล่องนี้ไปตั้งแต่รอบก่อน
--    แต่ฝาแฝด 768 มิติถูกลืม · เป็นรูปแบบ "แก้ตัวเดียวแล้วลืมคู่แฝด"
--
...
DROP INDEX IF EXISTS qf_real2_ivf;
DROP INDEX IF EXISTS qf_real2_hnsw;
DROP TABLE IF EXISTS qf_real2_q04;

CREATE TABLE qf_real2_q04 (
    kind        text,
    param       text,
    val         int,
    build_no    int,
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_real2_q04_measure(
...
BEGIN
    ...
    t0 := clock_timestamp();
    PERFORM (SELECT count(*) FROM (
                SELECT c.id FROM qf_real2 c ORDER BY c.embedding <=> q.embedding LIMIT 10) z)
    FROM qf_real2_q q;
    ms := extract(epoch FROM clock_timestamp() - t0) * 1000;

    SELECT * INTO r FROM qf_real2_recall_at(10);

    INSERT INTO qf_real2_q04 VALUES (
...
BEGIN
    ...
    FOR b IN 1..n LOOP
        EXECUTE 'DROP INDEX IF EXISTS qf_real2_ivf';
        EXECUTE 'CREATE INDEX qf_real2_ivf ON qf_real2 '
                'USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)';
        RAISE NOTICE '=== IVFFlat build %/% ===', b, n;
        FOREACH v IN ARRAY vals LOOP
            PERFORM qf_real2_q04_measure('ivfflat', 'ivfflat.probes', v, b);
        END LOOP;
    END LOOP;
    EXECUTE 'DROP INDEX IF EXISTS qf_real2_ivf';
...
BEGIN
    ...
    FOR b IN 1..n LOOP
        EXECUTE 'DROP INDEX IF EXISTS qf_real2_hnsw';
        EXECUTE 'CREATE INDEX qf_real2_hnsw ON qf_real2 '
                'USING hnsw (embedding vector_cosine_ops)';
        RAISE NOTICE '=== HNSW build %/% ===', b, n;
        FOREACH v IN ARRAY vals LOOP
            PERFORM qf_real2_q04_measure('hnsw', 'hnsw.ef_search', v, b);
        END LOOP;
    END LOOP;
    EXECUTE 'DROP INDEX IF EXISTS qf_real2_hnsw';
...
BEGIN
    SELECT count(*) INTO n_rows FROM qf_real2_q04;
    IF n_rows = 0 THEN
...
FROM qf_real2_q04 WHERE kind = 'ivfflat'
...
SELECT val                          AS "ef_search",
       ...
       round(avg(n_perfect))        AS "ครบ 10/10",
       round(avg(query_ms))         AS "200 query (ms)"
FROM qf_real2_q04 WHERE kind = 'hnsw'
GROUP BY val ORDER BY val;
