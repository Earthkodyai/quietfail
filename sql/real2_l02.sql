...
DROP TABLE IF EXISTS qf_l02r2_guard;
CREATE TABLE qf_l02r2_guard AS SELECT count(*) AS n FROM qf_real2;

DROP INDEX IF EXISTS qf_l02r2_idx;
DROP TABLE IF EXISTS qf_l02r2_obs;
DROP TABLE IF EXISTS qf_l02r2;

CREATE TABLE qf_l02r2_obs (
    stage text, dead_pct numeric, live bigint, asked int,
    got_index bigint, got_exact bigint, recall numeric, idx_size text
);

CREATE TABLE qf_l02r2 (id bigint PRIMARY KEY, embedding vector(768));
INSERT INTO qf_l02r2 SELECT id, embedding FROM qf_real2;
ALTER TABLE qf_l02r2 SET (autovacuum_enabled = false);
...
CREATE INDEX qf_l02r2_idx ON qf_l02r2 USING hnsw (embedding vector_cosine_ops);
ANALYZE qf_l02r2;
...
-- ------------------------------------------------------------
-- ตัววัด — query vector มาจาก qf_real2_q ซึ่งอยู่ **นอกตาราง** ที่ลบ
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION qf_l02r2_ask(p_limit int) RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE n bigint;
BEGIN
    EXECUTE format('SELECT count(*) FROM (SELECT id FROM qf_l02r2 '
...
CREATE OR REPLACE FUNCTION qf_l02r2_ask_exact(p_limit int) RETURNS bigint
LANGUAGE plpgsql AS $$
DECLARE n bigint;
BEGIN
...
CREATE OR REPLACE FUNCTION qf_l02r2_recall() RETURNS numeric
LANGUAGE plpgsql AS $$
DECLARE r numeric;
BEGIN
    SELECT round(avg(x), 4) INTO r FROM (
        SELECT (SELECT count(*) FROM unnest(t.ids) AS tid
                 WHERE tid = ANY (SELECT l.id FROM qf_l02r2 l
                                  ORDER BY l.embedding <=> q.embedding LIMIT 10)
               )::numeric / 10 AS x
        FROM qf_real2_truth t JOIN qf_real2_q q ON q.id = t.query_id
        WHERE t.k = 10) s;
    RETURN r;
END $$;

CREATE OR REPLACE FUNCTION qf_l02r2_note(p_stage text, p_dead numeric) RETURNS void
LANGUAGE plpgsql AS $$
BEGIN
    -- อุ่น cache ก่อนวัด (กฎเหล็กข้อ 8)
    PERFORM (SELECT count(*) FROM (
        SELECT id FROM qf_l02r2
        ORDER BY embedding <=> (SELECT embedding FROM qf_real2_q WHERE id=0) LIMIT 10) w);

    INSERT INTO qf_l02r2_obs
    SELECT p_stage, p_dead, count(*), 10,
           qf_l02r2_ask(10), qf_l02r2_ask_exact(10), qf_l02r2_recall(),
           pg_size_pretty(pg_relation_size('qf_l02r2_idx'))
    FROM qf_l02r2;
END $$;
...
-- ------------------------------------------------------------
SELECT qf_l02r2_note('ตั้งต้น', 0);

DELETE FROM qf_l02r2 WHERE id % 10 = 0;  SELECT qf_l02r2_note('ตาย 10%', 10);
DELETE FROM qf_l02r2 WHERE id % 10 = 1;  SELECT qf_l02r2_note('ตาย 20%', 20);
DELETE FROM qf_l02r2 WHERE id % 10 = 2;  SELECT qf_l02r2_note('ตาย 30%', 30);
DELETE FROM qf_l02r2 WHERE id % 10 = 3;  SELECT qf_l02r2_note('ตาย 40%', 40);
DELETE FROM qf_l02r2 WHERE id % 10 = 4;  SELECT qf_l02r2_note('ตาย 50%', 50);
DELETE FROM qf_l02r2 WHERE id % 10 = 5;  SELECT qf_l02r2_note('ตาย 60%', 60);
DELETE FROM qf_l02r2 WHERE id % 10 = 6;  SELECT qf_l02r2_note('ตาย 70%', 70);
DELETE FROM qf_l02r2 WHERE id % 10 = 7;  SELECT qf_l02r2_note('ตาย 80%', 80);
DELETE FROM qf_l02r2 WHERE id % 10 = 8;  SELECT qf_l02r2_note('ตาย 90%', 90);
...
SELECT 10   AS "ขอ", qf_l02r2_ask(10)   AS "ได้", qf_l02r2_ask_exact(10)   AS "เฉลย"
UNION ALL SELECT 40,   qf_l02r2_ask(40),   qf_l02r2_ask_exact(40)
UNION ALL SELECT 100,  qf_l02r2_ask(100),  qf_l02r2_ask_exact(100)
UNION ALL SELECT 1000, qf_l02r2_ask(1000), qf_l02r2_ask_exact(1000);
...
\qecho ''
\qecho '=== หลัง VACUUM ==='
VACUUM (VERBOSE false) qf_l02r2;
SELECT qf_l02r2_note('หลัง VACUUM', 90);
...
FROM qf_l02r2_obs ORDER BY ctid;
...
BEGIN
    SELECT got_exact INTO base FROM qf_l02r2_obs WHERE stage = 'ตั้งต้น';
    IF base <> 10 THEN
        RAISE EXCEPTION 'ข้อ 1 ตก: เฉลยตอนตั้งต้นได้ % (ต้อง 10)', base;
    END IF;
    RAISE NOTICE '[1/2] OK เฉลยตอนตั้งต้นได้ครบ 10';

    SELECT count(*) INTO n_bad FROM qf_l02r2_obs WHERE got_index < got_exact;
    RAISE NOTICE '[2/2] ขั้นที่ index ให้ไม่ครบ: % จาก % ขั้น',
        n_bad, (SELECT count(*) FROM qf_l02r2_obs);
END $$;
...
BEGIN
    SELECT n INTO n_before FROM qf_l02r2_guard;
    SELECT count(*) INTO n_now FROM qf_real2;
...
DROP TABLE IF EXISTS qf_l02r2_guard;
DROP TABLE IF EXISTS qf_l02r2_obs CASCADE;
